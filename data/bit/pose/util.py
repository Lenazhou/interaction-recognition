...
def print_image_color_u(action_txt,path_image,out_path,group_p,label,bbox):
    font = cv2.FONT_HERSHEY_SIMPLEX
    image = cv2.imread(path_image)
    for g,l in zip(group_p,label):
        if l != 6:
            ...
            print([box[0],box[1]-25,box[0]+50,box[1]])
            #image = put_mask(image,[box[0],box[1]-25,box[0]+50,box[1]])
            b = [int(box[0]), int(box[1] - 50), int(box[0] + 90), int(box[1])]
            image = put_mask(image, b)
            image = cv2.putText(image, action_txt[int(l)], (box[0] + 5, box[1] - 5), font, 1.8, (0, 0, 0), 3)
    cv2.imwrite(out_path,image)
# def get_files_id(split,a):
#     action = ['bend', 'box', 'handshake', 'hifive', 'hug', 'kick', 'pat', 'push']
#
#     seq = ['{}/{}_{:04d}'.format(i, i, j) for i in action for j in a]  # first 34 used as train set
#
#
#     #tst_seq = ['{}/{}_{:04d}'.format(i, i, j) for i in action for j in b]  # rest 16 used as test set
#
