...
def center_resize_img_ci(image,coor,skeleton):
    #image[C,H,W]
    image = image[:,14:203,24:360] #images[3,210,280]
    image = sktsf.resize(image,(3,216,384),mode='reflect',anti_aliasing=False)
    coor[:, 0] = (coor[:, 0] - 24) * 8.0/7.0
    coor[:, 1] = (coor[:, 1] - 14) * 8.0/7.0
    coor[:, 2] = (coor[:, 2] - 24) * 8.0/7.0
    coor[:, 3] = (coor[:, 3] - 14) * 8.0/7.0

    skeleton = skeleton[:, 14:203,24:360]
    skeleton = sktsf.resize(skeleton,(36,216,384),mode='reflect',anti_aliasing=False)
...
def random_crop_resize(img,coor,skeleton,crop_random=False):
    ...
    if center_crop:
        img,coor,skeleton=center_resize_img_ci(img,coor,skeleton)
        coor=np.array(coor)
...
def get_box(box1,box2):
    ...
    return torch.Tensor([0,x_min,y_min,x_max,y_max])