...
def flip_skeleton(skeleton,y_flip=False,x_flip=False):
...
def draw_center_crop(coor):
    ...
    for k,i in enumerate(coor[:,3]):
        if i>520:
            coor[k,3]=520
    return coor
def center_resize_img(image,coor,skeleton):
    #image[C,H,W]
    image = image[:,15:225,20:300] #images[3,210,280]
    image = sktsf.resize(image,(3,240,320),mode='reflect',anti_aliasing=False)
    coor[:, 0] = (coor[:, 0] - 20) * 8.0 / 7.0
    coor[:, 1] = (coor[:, 1] - 15) * 8.0 / 7.0
    coor[:, 2] = (coor[:, 2] - 20) * 8.0 / 7.0
    coor[:, 3] = (coor[:, 3] - 15) * 8.0 / 7.0

    skeleton = skeleton[:, 15:225, 20:300]
    skeleton = sktsf.resize(skeleton,(10,240,320),mode='reflect',anti_aliasing=False)

    return image,coor,skeleton

def center_resize_img_ut(image,coor,skeleton):
    #image[C,H,W]
    image = image[:,20:220,30:330] #images[3,210,280]
    image = sktsf.resize(image,(3,240,360),mode='reflect',anti_aliasing=False)
    coor[:, 0] = (coor[:, 0] - 30) * 1.2
    coor[:, 1] = (coor[:, 1] - 20) * 1.2
    coor[:, 2] = (coor[:, 2] - 30) * 1.2
    coor[:, 3] = (coor[:, 3] - 20) * 1.2

    skeleton = skeleton[:, 20:220,30:330]
    skeleton = sktsf.resize(skeleton,(10,240,360),mode='reflect',anti_aliasing=False)

    return image,coor,skeleton

def random_crop_resize(img,coor,skeleton,crop_random=False):
    center_crop=False
    if crop_random:
        center_crop = random.choice([True, False])
    if center_crop:
        img,coor,skeleton=center_resize_img(img,coor,skeleton)
        coor=np.array(coor)
    return img,coor,skeleton

def random_crop_resize_ut(img,coor,skeleton,crop_random=False):
    center_crop=False
    if crop_random:
        center_crop = random.choice([True, False])
    if center_crop:
        img,coor,skeleton=center_resize_img_ut(img,coor,skeleton)
        coor=np.array(coor)
    return img,coor,skeleton
...
def put_mask(image,bbox1):
    ...
    try:
        ...
        return None
import os
def print_image(action_txt,path_image,out_path,group_p,group_num,label,bbox):
    font = cv2.FONT_HERSHEY_SIMPLEX
    image = cv2.imread(path_image)
    for g,l in zip(group_p[:group_num],label[:group_num]):
        if l != 6:
            box1 = bbox[g[0]]
            box2 = bbox[g[1]]
            box = get_box(box1,box2)
            cv2.rectangle(image, (box[0], box[1]), (box[2], box[3]), (0, 0, 0), 2)
            #image = put_mask(image,[box[0],box[1]-25,box[0]+50,box[1]])
            b = [int(box[0]),int(box[1]-30),int(box[0]+50),int(box[1])]
            image = put_mask(image, b)
            image= cv2.putText(image,action_txt[int(l)] , (box[0]+5, box[1]-5), font, 1, (0, 0, 0), 2)
    cv2.imwrite(out_path,image)

def get_files_id(split):
    action = ['bend', 'box', 'handshake', 'hifive', 'hug', 'kick', 'pat', 'push']
    trn_seq = ['{}/{}_{:04d}'.format(i, i, j) for i in action for j in range(1, 35)]  # first 34 used as train set
    tst_seq = ['{}/{}_{:04d}'.format(i, i, j) for i in action for j in range(35, 51)]  # rest 16 used as test set
    path = '/home/zhenghua/zj/dataset/BIT/all'
    trn_i = []
    for i in trn_seq:
        path_a = os.path.join(path,i)
        for k in os.listdir(path_a):
            trn_i.append(i+'/'+k)
    tst_i = []
    for i in tst_seq:
        path_a = os.path.join(path,i)
