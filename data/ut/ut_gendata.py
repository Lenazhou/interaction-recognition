...
class UTDataset:
    ...
    def __init__(self, opt, split='train_name'):
        id_list_file = os.path.join(
            opt.data_dir, 'data/ut/{0}.npy'.format(split))
    ...
    def get_example(self, i):
        ...
        id_2=(id_.split('-')[1]).replace('.json','.npy')

        skeleton = np.load(os.path.join(self.dataset_dir,'pose',seq_num,id_2))
        ...
        img=read_img(seq_num,frame_num,self.opt)
        ...
        img = np.array(img,dtype=np.float32) #resize
...
def read_img(seq_num,frame_num,opt):
    path = os.path.join(opt.dataset_dir,'frame',seq_num,frame_num+'.jpg')
    f = Image.open(path)
    img = f.convert('RGB')
    img = np.asarray(img)
    img = img.transpose((2,0,1))
    img = sktsf.resize(img, (3,240,360),mode='reflect',anti_aliasing=False)
    return img
